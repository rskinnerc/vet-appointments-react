...
const Appointment = () => {
  ...
  if (!user) {
    return (
      <SignInWarning pageTitle="MAKE AN APPOINTMENT" />
    );
  }
  ...
  if (doctors.length === 0) {
    return (
      <EmptyListWarning
        ...
        bodyText="There are no doctors to book an appointment with. Please create one"
        linkText="Add Doctor"
      ...
      // There are no doctors to book an appointment with. Please create one.</p>
      //   <Link to="/new-doctor" className="bg-amber-500 mx-auto
      // my-4 h-10 px-24 self-center rounded-full text-white
      // font-semibold flex items-center justify-center gap-2">Add a Doctor</Link>
      // </section>
    );
  }
  ...
  return (
    <section className="flex flex-col items-center w-full lg:w-10/12 pb-20">
      <h1 data-testid="title" className="text-3xl md:text-4xl lg:text-6xl font-bold text-center mt-16 lg:self-end lg:mr-24 lg:mt-36">MAKE AN APPOINTMENT</h1>
      <div className="w-full relative flex justify-around mt-14 md:mt-24">
        <form onSubmit={(e) => newAppointment(e)} action="" className="w-full flex flex-col items-center mt-3">
          ...
          <button type="submit" data-testid="submit" className="mt-20 w-1/2 text-center md:w-1/5 h-12 bg-citrus-500 hover:bg-citrus-600 hover:shadow-md text-white px-6 shadow-lg rounded-full font-semibold focus:bg-citrus-700 transition duration-150 ease-in-out">Add Appointment</button>
        </form>
      </div>
    </section>
  );
